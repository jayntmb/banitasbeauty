<footer class="footer-page">
    <div class="first-part">
      <div class="container">
        <div class="row g-3">
          <div class="col-lg-4">
            <div class="d-flex flex-column gap-2 mb-lg-4 mb-3">
              <a href="tel:+2438912340000" class="link-contact d-inline-flex gap-3 align-items-center">
                <svg viewBox="0 0 24 24" width="512" height="512">
                  <path
                    d="M15.045,0c-.027-.001-6.063-.001-6.09,0-2.736,.024-4.955,2.258-4.955,4.999v14c0,2.757,2.243,5,5,5h6c2.757,0,5-2.243,5-5V5C20,2.258,17.781,.025,15.045,0Zm2.955,18.999c0,1.654-1.346,3-3,3h-6c-1.654,0-3-1.346-3-3V5c0-1.453,1.038-2.667,2.411-2.942l.694,1.389c.169,.339,.516,.553,.895,.553h4c.379,0,.725-.214,.895-.553l.694-1.389c1.373,.274,2.411,1.489,2.411,2.942v14Zm-5,1h-2c-.552,0-1-.448-1-1h0c0-.552,.448-1,1-1h2c.552,0,1,.448,1,1h0c0,.552-.448,1-1,1Z" />
                </svg>

                <span>+243840035211</span>
              </a>
              <a href="mailto:banitasbeauty@gmail.com" class="link-contact d-inline-flex gap-3 align-items-center">
                <svg viewBox="0 0 24 24" width="512" height="512">
                  <path
                    d="M19,1H5A5.006,5.006,0,0,0,0,6V18a5.006,5.006,0,0,0,5,5H19a5.006,5.006,0,0,0,5-5V6A5.006,5.006,0,0,0,19,1ZM5,3H19a3,3,0,0,1,2.78,1.887l-7.658,7.659a3.007,3.007,0,0,1-4.244,0L2.22,4.887A3,3,0,0,1,5,3ZM19,21H5a3,3,0,0,1-3-3V7.5L8.464,13.96a5.007,5.007,0,0,0,7.072,0L22,7.5V18A3,3,0,0,1,19,21Z" />
                </svg>
                <span>banitasbeauty@gmail.com</span>
              </a>
            </div>
            <p class="paragraph sm mb-0">
              Assistance en ligne 24/7 | Live M-F 9am-6pm
              N°876. Av. De la justice, Immeuble Rose, Local 6, Niveau 1
            </p>
          </div>
          <div class="col-lg-5 ms-auto">
            <p class="paragraph sm mb-1">Suivez-nous sur nos réseaux sociaux</p>
            <div class="d-flex align-items-center gap-2 mb-lg-4 mb-3">
              <a href="#" class="link-network" target="_blank">
                <svg viewBox="0 0 24 24" width="512" height="512">
                  <g id="WA_Logo">
                    <g>
                      <path style="fill-rule:evenodd;clip-rule:evenodd;"
                        d="M20.463,3.488C18.217,1.24,15.231,0.001,12.05,0    C5.495,0,0.16,5.334,0.157,11.892c-0.001,2.096,0.547,4.142,1.588,5.946L0.057,24l6.304-1.654    c1.737,0.948,3.693,1.447,5.683,1.448h0.005c6.554,0,11.89-5.335,11.893-11.893C23.944,8.724,22.708,5.735,20.463,3.488z     M12.05,21.785h-0.004c-1.774,0-3.513-0.477-5.031-1.378l-0.361-0.214l-3.741,0.981l0.999-3.648l-0.235-0.374    c-0.99-1.574-1.512-3.393-1.511-5.26c0.002-5.45,4.437-9.884,9.889-9.884c2.64,0,5.122,1.03,6.988,2.898    c1.866,1.869,2.893,4.352,2.892,6.993C21.932,17.351,17.498,21.785,12.05,21.785z M17.472,14.382    c-0.297-0.149-1.758-0.868-2.031-0.967c-0.272-0.099-0.47-0.149-0.669,0.148s-0.767,0.967-0.941,1.166    c-0.173,0.198-0.347,0.223-0.644,0.074c-0.297-0.149-1.255-0.462-2.39-1.475c-0.883-0.788-1.48-1.761-1.653-2.059    s-0.018-0.458,0.13-0.606c0.134-0.133,0.297-0.347,0.446-0.521C9.87,9.97,9.919,9.846,10.019,9.647    c0.099-0.198,0.05-0.372-0.025-0.521C9.919,8.978,9.325,7.515,9.078,6.92c-0.241-0.58-0.486-0.501-0.669-0.51    C8.236,6.401,8.038,6.4,7.839,6.4c-0.198,0-0.52,0.074-0.792,0.372c-0.272,0.298-1.04,1.017-1.04,2.479    c0,1.463,1.065,2.876,1.213,3.074c0.148,0.198,2.095,3.2,5.076,4.487c0.709,0.306,1.263,0.489,1.694,0.626    c0.712,0.226,1.36,0.194,1.872,0.118c0.571-0.085,1.758-0.719,2.006-1.413c0.248-0.694,0.248-1.29,0.173-1.413    C17.967,14.605,17.769,14.531,17.472,14.382z" />
                    </g>
                  </g>
                </svg>
              </a>
              <a href="#" class="link-network" target="_blank">
                <svg version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 24 24" width="512" height="512">
                  <g>
                    <path d="M12,2.162c3.204,0,3.584,0.012,4.849,0.07c1.308,0.06,2.655,0.358,3.608,1.311c0.962,0.962,1.251,2.296,1.311,3.608   c0.058,1.265,0.07,1.645,0.07,4.849c0,3.204-0.012,3.584-0.07,4.849c-0.059,1.301-0.364,2.661-1.311,3.608   c-0.962,0.962-2.295,1.251-3.608,1.311c-1.265,0.058-1.645,0.07-4.849,0.07s-3.584-0.012-4.849-0.07   c-1.291-0.059-2.669-0.371-3.608-1.311c-0.957-0.957-1.251-2.304-1.311-3.608c-0.058-1.265-0.07-1.645-0.07-4.849   c0-3.204,0.012-3.584,0.07-4.849c0.059-1.296,0.367-2.664,1.311-3.608c0.96-0.96,2.299-1.251,3.608-1.311   C8.416,2.174,8.796,2.162,12,2.162 M12,0C8.741,0,8.332,0.014,7.052,0.072C5.197,0.157,3.355,0.673,2.014,2.014   C0.668,3.36,0.157,5.198,0.072,7.052C0.014,8.332,0,8.741,0,12c0,3.259,0.014,3.668,0.072,4.948c0.085,1.853,0.603,3.7,1.942,5.038   c1.345,1.345,3.186,1.857,5.038,1.942C8.332,23.986,8.741,24,12,24c3.259,0,3.668-0.014,4.948-0.072   c1.854-0.085,3.698-0.602,5.038-1.942c1.347-1.347,1.857-3.184,1.942-5.038C23.986,15.668,24,15.259,24,12   c0-3.259-0.014-3.668-0.072-4.948c-0.085-1.855-0.602-3.698-1.942-5.038c-1.343-1.343-3.189-1.858-5.038-1.942   C15.668,0.014,15.259,0,12,0z"/>
                    <path d="M12,5.838c-3.403,0-6.162,2.759-6.162,6.162c0,3.403,2.759,6.162,6.162,6.162s6.162-2.759,6.162-6.162   C18.162,8.597,15.403,5.838,12,5.838z M12,16c-2.209,0-4-1.791-4-4s1.791-4,4-4s4,1.791,4,4S14.209,16,12,16z"/>
                    <circle cx="18.406" cy="5.594" r="1.44"/>
                  </g>
                </svg>
              </a>
            </div>
            <p class="paragraph sm mb-0">
              Vous pouvez vous désinscrire à tout moment. <br>
              Vous trouverez pour cela nos informations de
              contact dans les conditions d'utilisation du site.
            </p>
          </div>
        </div>
        <hr class="my-lg-4 my-3">
      </div>
    </div>
    <div class="second-part">
      <div class="container">
        <div class="row g-3">
          <div class="col-lg-2">
            <a href="#" class="logo">
              <img src="images/logos/logo-banitasbeauty.png" class="logo-dark" alt="logo de Banitas">
            </a>
            <p class="paragraph xm mb-0">
              Localisation: Kinshasa, RDC
            </p>
          </div>
          <div class="col-lg-4">
            <p class="paragraph sm">
              Nous mettons à votre disposition les meilleurs
              produits cosmétiques de haute qualité destinés
              à protéger et à embellir votre peau
            </p>
          </div>
          <div class="col-lg-5 col-xl-5 ms-auto">
            <div class="row g-2">
              <div class="col-lg-4">
                <div class="d-flex flex-column gap-2">
                  <a href="#" class="link">
                    <span>Delivery</span>
                    <span>Delivery</span>
                    <span>Delivery</span>
                  </a>
                  <a href="#" class="link">
                    <span>Secure payment </span>
                    <span>Secure payment </span>
                    <span>Secure payment </span>
                  </a>
                  <a href="#" class="link">
                    <span>Identifiant </span>
                    <span>Identifiant </span>
                    <span>Identifiant </span>
                  </a>
                </div>
              </div>
              <div class="col-lg-4">
                <div class="d-flex flex-column gap-2">
                  <a href="#" class="link">
                    <span>Legal Notice</span>
                    <span>Legal Notice</span>
                    <span>Legal Notice</span>
                  </a>
                  <a href="#" class="link">
                    <span>Contactez-nous</span>
                    <span>Contactez-nous</span>
                    <span>Contactez-nous</span>
                  </a>
                  <a href="#" class="link">
                    <span>Mon compte</span>
                    <span>Mon compte</span>
                    <span>Mon compte</span>
                  </a>
                </div>
              </div>
              <div class="col-lg-4">
                <div class="d-flex flex-column gap-2">
                  <a href="#" class="link">
                    <span>Entreprise</span>
                    <span>Entreprise</span>
                    <span>Entreprise</span>
                  </a>
                  <a href="#" class="link">
                    <span>Plan du site </span>
                    <span>Plan du site </span>
                    <span>Plan du site</span>
                  </a>
                  <a href="#" class="link">
                    <span>Boutique </span>
                    <span>Boutique </span>
                    <span>Boutique </span>
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </footer>
