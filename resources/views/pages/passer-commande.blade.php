                                ...
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-lg-2">
                                        <div class="img-product">
                                            <a href="{{ route('detal-product', [$commande->produit->id]) }}">
                                                <img src="{{ asset('assets/images/produits/'.$commande->produit->image) }}" alt="">
                                            </a>
                                        </div>
                                    </div>
                                    <div class="col-lg-4">
                                    ...
                                    <div class="col-lg-3">
                                        <div class="d-flex justify-content-center align-items-center">
                                            ...
                                            {{-- <input type="hidden" name="commande[]" value="{{$commande->produit->nom}}  Qté : {{$commande->quantite}}  P.U: {{$commande->prix}} Total:{{$commande->prix_total}}  "/> --}}
                                            <input type="hidden" name="commande[{{$key}}][nom]" value="{{$commande->produit->nom}}">
                                            <input type="hidden" name="commande[{{$key}}][quantite]" value="{{$commande->quantite}}">
                                            <input type="hidden" name="commande[{{$key}}][prix]" value="{{$commande->prix}}">
                                            <input type="hidden" name="commande[{{$key}}][symbole]" value="{{$commande->devise->symbole}}">
                                            <input type="hidden" name="commande[{{$key}}][prix_total]" value="{{$commande->prix_total}}">

                                            @livewire('commande',['commande' => $commande])
                                        </div>
                                    </div>
                                    <div class="col-lg-2 ">
                                        <a href="{{ route('commande.delete', [$commande->id]) }}" class="btn btn-delete">Supprimer</a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
